...
#include "service/http/base_app.hpp"
...
class WebApp : public BaseApp {
public:
  WebApp();

private:
  bool prepare() override;
  std::pair<std::string, int> host_port() override;
  std::unique_ptr<Router> router() override;
  //
  bool init_db() const;
  bool load_hn_index() const;
...
inline WebApp::WebApp() {
  conf_ptr_ = Context::singleton()->with_config();
  MapBasedRouterConf router_conf{};
  router_conf.func_log_req = log_req;
  router_conf.global_rate_limit = conf_ptr_->server_conf.global_rate_limit;
  router_conf.per_client_rate_limit = conf_ptr_->server_conf.per_client_rate_limit;
  router_ptr_ = std::make_unique<MapBasedRouter>(router_conf);
}

inline bool WebApp::prepare() {
  if (!init_db()) {
  ...
  spdlog::info("change working dir from {} to {}", origin_wd.string(), current_path().string());
  //
  router_ptr_->add_routes({
...
inline bool WebApp::init_db() const {
  auto& db = storage::LocalSqlite::singleton();
  return db.open(conf_ptr_->storage.db_file_path, conf_ptr_->storage.init_sql);
}

inline bool WebApp::load_hn_index() const {
  auto hn_data_path = toml::find_or_default<std::string>(conf_ptr_->raw_toml_, "hn", "data_path");
  auto& hn_hnsw = storage::HackNewsHnsw::singleton();
  hn_hnsw.data_path(hn_data_path);
  return hn_hnsw.load();
}

inline std::pair<std::string, int> WebApp::host_port() {
  return std::make_pair(FLAGS_host, FLAGS_port);
}

inline std::unique_ptr<Router> WebApp::router() {
  return std::unique_ptr<Router>(router_ptr_.release());
}
}