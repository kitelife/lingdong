...
inline MmdbRecordPtr MaxMindGeoLite2Db::query_by_ip(const std::string& ip) const {
  ...
  if (gai_error == 0 && MMDB_SUCCESS == mmdb_error && res.found_entry) {
    ...
    if (MMDB_SUCCESS == MMDB_get_value(&res.entry, &city_name_en_entry, "city", "names", "en", nullptr) && city_name_en_entry.has_data) {
    ...
    if (MMDB_SUCCESS == MMDB_get_value(&res.entry, &country_name_en_entry, "country", "names", "en", nullptr) && country_name_en_entry.has_data) {
    ...
    if (MMDB_SUCCESS == MMDB_get_value(&res.entry, &continent_name_en_entry, "continent", "names", "en", nullptr) && continent_name_en_entry.has_data) {
...
inline bool MaxMindGeoLite2Db::check_download() {
  ...
  auto progress_cb = [&last_download_progress](cpr::cpr_pf_arg_t downloadTotal, cpr::cpr_pf_arg_t downloadNow, cpr::cpr_pf_arg_t uploadTotal,
    cpr::cpr_pf_arg_t uploadNow, intptr_t userdata) -> bool {
